...
select (select coalesce(max(id)+1,1) from modulos),'Rede de Parceria (Usuários) - Consulta',2,1
where not exists (select 1 from modulos where descricao = 'Rede de Parceria (Usuários) - Consulta');

insert into modulos (id, descricao, idmodcoresso,idacao) 	
select (select max(id)+1 from modulos),'Rede de Parceria (Usuários) - Inclusão',2,2
where not exists (select 1 from modulos where descricao = 'Rede de Parceria (Usuários) - Inclusão');

insert into modulos (id, descricao, idmodcoresso,idacao) 	
select (select max(id)+1 from modulos),'Rede de Parceria (Usuários) - Exclusão',2,3
where not exists (select 1 from modulos where descricao = 'Rede de Parceria (Usuários) - Exclusão');

insert into modulos (id, descricao, idmodcoresso,idacao) 	
select (select max(id)+1 from modulos),'Rede de Parceria (Usuários) - Alteração',2,4
