...
select (select coalesce(max(id)+1,1) from modulos),'Proposta - Consulta',3,1
where not exists (select 1 from modulos where descricao = 'Proposta - Consulta');

insert into modulos (id, descricao, idmodcoresso,idacao) 	
select (select max(id)+1 from modulos),'Proposta - Inclusão',3,2
where not exists (select 1 from modulos where descricao = 'Proposta - Inclusão');

insert into modulos (id, descricao, idmodcoresso,idacao) 	
select (select max(id)+1 from modulos),'Proposta - Exclusão',3,3
where not exists (select 1 from modulos where descricao = 'Proposta - Exclusão');

insert into modulos (id, descricao, idmodcoresso,idacao) 	
select (select max(id)+1 from modulos),'Proposta - Alteração',3,4
