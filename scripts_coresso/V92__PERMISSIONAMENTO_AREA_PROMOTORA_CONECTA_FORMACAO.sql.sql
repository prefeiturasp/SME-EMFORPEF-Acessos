...
BEGIN TRY
	BEGIN TRAN
		...
		--> Cadastros
		if not exists(select * from SYS_Modulo where sis_id = @sistema_id and mod_idPai is null and mod_nome = 'Cadastros')
		begin
			...
			insert into SYS_Modulo(sis_id, mod_id, mod_nome, mod_idPai,mod_auditoria,mod_situacao, mod_dataCriacao) 
		  	values(@sistema_id,(select coalesce(Max(mod_id)+1,1) from SYS_Modulo where sis_id = @sistema_id), 'Cadastros', null,0,1, getdate());
		end
		
		select @mod_id_pai_cadastros = mod_id from SYS_Modulo where sis_id = @sistema_id and mod_idPai is null and mod_nome = 'Cadastros';
		
		--> Área promotora
		if not exists(select * from SYS_Modulo where sis_id = @sistema_id and mod_idPai = @mod_id_pai_cadastros and mod_nome = 'Área promotora')
		begin
			print 'inserindo módulo Área promotora'
			insert into SYS_Modulo(sis_id, mod_id, mod_nome, mod_idPai,mod_auditoria,mod_situacao, mod_dataCriacao) 
		    values(@sistema_id,(select Max(mod_id)+1 from SYS_Modulo where sis_id = @sistema_id), 'Área promotora', @mod_id_pai_cadastros,0,1, getdate());
		end
		
		select @mod_id = mod_id from SYS_Modulo where sis_id = @sistema_id and mod_idPai = @mod_id_pai_cadastros and mod_nome = 'Área promotora';					 
		------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
-- ajusta visualização grupos DICEU (DRE) e DIPED (DRE)
update SYS_Grupo set vis_id = 3
where sis_id = @sistema_id and (gru_nome = 'DICEU (DRE)' or gru_nome = 'DIPED (DRE)');
